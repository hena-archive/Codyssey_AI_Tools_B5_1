...
- min heap 구현
- node 테스트, double linked list 테스트
- min heap 테스트
...
Redis의 핵심 기능을 직접 구현하며 CLI 기반 Mini Redis를 완성하기.
- 해시맵
- 이중 연결 리스트
- 힙
위와 같은 내장 자료형 내부 동작 원리를 이해하기.
LRU 방식 이해.
TTL 만료 시간 관리 직접 구현
...
| 분류 | 명령어 | 설명 |
|---|---|---|
| String 타입 명령어 | `SET` | 키에 값 저장 (성공 시 내부적으로 LRU 추적 업데이트) |
| String 타입 명령어 | `GET` | 키의 값 조회 (성공 시 내부적으로 LRU 추적 업데이트) |
| String 타입 명령어 | `DEL` | 키 삭제 |
| String 타입 명령어 | `EXISTS` | 키 존재 여부 확인 |
| String 타입 명령어 | `DBSIZE` | 전체 키 개수 반환 |
| String 타입 명령어 | `KEYS` | 전체 키 목록 출력 (패턴 매칭은 구현하지 않음) |
| 메모리 관리 명령어 | `CONFIG SET maxmemory` | 최대 메모리 제한 설정 (바이트 단위) |
| 메모리 관리 명령어 | `INFO memory` | 현재 메모리 사용량, 제한, 제거된 키 개수 확인 |
| TTL 관리 명령어 | `EXPIRE` | 키의 만료 시간 설정 (초 단위) |
| TTL 관리 명령어 | `TTL` | 키의 남은 만료 시간 조회 |
| CLI 인터페이스 | x | REPL 환경 |
...
LRU 제거 규칙
maxmemory > 0이고, SET 이후 used_memory가 maxmemory를 초과하면 used_memory가 maxmemory 이하가 될 때까지 “가장 오래 사용되지 않은 키(LRU)”부터 제거한다
제거된 키는 evicted_keys에 누적 카운트한다
